...
//		System.out.println(path);
	...
	private static void getMaxPath(int[][] mountain){
		int height = mountain.length;
		int[][] mountainResult = new int[height][];
		String[][] pathResult = new String[height][];
		int j=height-2;
		mountainResult[height-1] = mountain[height-1];
		pathResult[height-1] = new String[height-1];
		do {
			int i = 0;
			mountainResult[j] = new int[mountain[j].length];
			pathResult[j] = new String[mountain[j].length];
			do {
				int max;
				String dest;
				if ((max = Math.max(mountainResult[j + 1][i], mountainResult[j + 1][i + 1])) == mountainResult[j + 1][i]) {
					pathResult[j][i] = "left, " + pathResult[j + 1][i];
				}
				else {
					pathResult[j][i] = "right, " + pathResult[j + 1][i+1];
				}
				mountainResult[j][i] = max + mountain[j][i];
				i++;
			} while (i <= j);
			j--;
		}while(j>=0);
		System.out.println(mountainResult[0][0]);
		System.out.println(pathResult[0][0]);
	}
}
